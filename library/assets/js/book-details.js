document.addEventListener('DOMContentLoaded', () => {
  ...
  // جلب تفاصيل الكتاب
  fetch(`http://localhost:5013/api/Book/details/${bookId}`)
    .then(res => {
    ...
    .then(book => {
      ...
      const authorImage = author?.authorImage
        ? `http://localhost:5013${author.authorImage}`
      ...
      // جلب المراجعات
      fetch(`http://localhost:5013/api/Book/reviews/${bookId}`)
        .then(res => {
      ...
      fetch(`http://localhost:5013/api/Book/by-category/${book.categoryId}`)
        .then(res => {
        ...
        .then(data => {
          ...
          books.forEach(book => {
            ...
            img.src = `http://localhost:5013/${book.bookImage}`;
            img.alt = book.title;
