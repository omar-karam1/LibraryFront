document.getElementById("registerForm").addEventListener("submit", async function(e) {
  ...
  try {
    const [userNameRes, emailRes] = await Promise.all([
      fetch(`http://localhost:5013/api/User/IsUserNameUnique?userName=${encodeURIComponent(userName)}`),
      fetch(`http://localhost:5013/api/User/IsEmailUnique?email=${encodeURIComponent(email)}`)
    ]);
