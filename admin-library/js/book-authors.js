...
async function loadBooks() {
    try {
        const response = await fetch('http://localhost:5013/api/Book/allBooks');
...
async function loadAuthors() {
    try {
        const response = await fetch('http://localhost:5013/api/Book/GetAllAuthors');
        const data = await response.json();
...
async function linkBookToAuthor() {
    ...
    try {
        const response = await fetch('http://localhost:5013/api/Admin/AddAuthorsToBook', {
            method: 'POST',
...
async function loadBookAuthorLinks() {
    try {
        const response = await fetch('http://localhost:5013/api/Admin/all-links'); // غيّر هذا الرابط حسب API الحقيقي
        const data = await response.json();
...
async function unlinkBookAuthor(bookId, authorId) {
    ...
    try {
        const response = await fetch(`http://localhost:5013/api/Admin/unlink?bookId=${bookId}&authorId=${authorId}`, {
            method: 'DELETE'
        });
