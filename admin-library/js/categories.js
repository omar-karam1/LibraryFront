...
document.querySelector("#category-modal form").addEventListener("submit", function (e) {
    ...
    fetch(`http://localhost:5013/api/Admin/AddNewCategorie?categoryName=${encodeURIComponent(categoryName)}`, {
        method: 'POST'
    })
    .then(res => {
        if (res.ok) {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        fetch("http://localhost:5013/api/Admin/all-categories")
            .then(res => res.json())
            .then(data => {
                // إفراغ الجدول الحالي
                tableBody.innerHTML = "";
    ...
    function deleteCategory(categoryId) {
        if (confirm("هل أنت متأكد أنك تريد حذف هذا التصنيف؟")) {
            fetch(`http://localhost:5013/api/Admin/delete-category/${categoryId}`, {
                method: 'DELETE'
            })
            .then(res => {
                if (res.ok) {
    ...
    function editCategory(categoryId, currentName) {
        ...
        if (newName && newName !== currentName) {
            fetch(`http://localhost:5013/api/Admin/update-category?CategoryID=${categoryId}&NewCategory=${encodeURIComponent(newName)}`, {
                method: 'PUT'
            })
            .then(res => {
                if (res.ok) {
